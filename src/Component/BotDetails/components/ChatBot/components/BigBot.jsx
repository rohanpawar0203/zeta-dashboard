  ...
  return (
    <Fragment>
       ...
       <div style={{height: '250px', padding: '15px', border: '1px solid none'}} className='w-100'>
        <div  className="d-flex jusify-content-end align-items-center mb-1">
          <img src="https://bot.writesonic.com/_next/image?url=https%3A%2F%2Fwritesonic-frontend.s3.us-east-1.amazonaws.com%2Ffrontend-assets%2Ftemplates-new%2FBotsonicNew.png&w=96&q=75" alt="" 
           ...
           height='30px'
           />
          <p style={{background: 'whitesmoke'}} className='mx-2 p-2 rounded'>{myBot?.welcomeMessage}</p>
        </div>
        {messages.map((ele, ind) => (
          <div key={ind}  className={`d-flex jusify-content-end align-items-center mb-1 ${ele?.user ? 'flex-row-reverse' : 'flex-row'}`}>
          <img src={`${ele?.user ? myBot.companyLogo : botAvatar}`} alt={`${ele?.user ? 'user avatar' : 'bot avatar'}`}
           width='30px'
           height='30px'
           />
          <p style={{background: 'whitesmoke'}} className='mx-2 p-2 rounded'>{ele?.text}</p>
        </div>
        ))}
       </div>
       </ScrollBar>
      ...
      <div style={{zIndex: '2', paddding:'5px', borderRadius: '25px', border: `1px solid ${myBot?.accentColor}`}} className='d-flex'>
            ...
            }} style={{border: 'none', borderRadius: '25px'}} className="form-control" type="text" aria-label="Amount (to the nearest dollar)" placeholder='Send Message...'/>
